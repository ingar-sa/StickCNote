...
isa_internal void UpdateBg(u32_argb NewColor) { Bg.Color = NewColor; }

isa_internal void UpdateBgColorWhenDragging(bg_landscape *Landscape, i64 x,
                                            i64 y) {
  Landscape->x += x;
  Landscape->y += y;
...
isa_internal scn_state *InitScnState(scn_mem *Mem) {
  scn_state *State = (scn_state *)Mem->Permanent;
  if (!Mem->Initialized) {
    State->Arena = IsaArenaCreate((u8 *)Mem->Permanent + sizeof(scn_state),
                                  Mem->PermanentMemSize - sizeof(scn_state));
...
extern "C" SEED_RAND_PCG(SeedRandPcg) { SeedRandPcg_(Seed); }
